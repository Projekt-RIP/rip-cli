...
function generatePaths() {
    ...
    g_uids.forEach(uid => {
        ...
        const path = "./collection/" + url_path.split(".").reverse().join("/") + "/index.spec.js"
...
function performDeletion() {
    ...
    g_uids.forEach(uid => {
        ...
        if (g_paths[uid] == undefined) {
            ...
            generateLog(uid,`Sorry, but there are no intstructions for ${data.url} yet`, 1)
            finished(uid)
        } else {
            ...
            g_running[uid] = cypress.run({
                ...
                if (results.totalFailed != 0) {
                    ...
                    generateLog(uid, "Sorry, but we weren't able to delete this account!", 1)
                } else if(results.totalPassed != 0) {
                    // Greate this worked pefektly ... hopefully ':)
                    generateLog(uid, "The Account has been deleted!", 0)
                } else {
